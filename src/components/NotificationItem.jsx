...
export default function NotificationItem({ title, message, timestamp }) {
  return (
    <article className="group flex items-start gap-3 rounded-2xl border border-[var(--stroke)] bg-[var(--surface)] p-4 transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <div>
        ...
        <p className="mt-2 text-xs text-[var(--muted)]">{timestamp}</p>
      </div>
    </article>
  )
}
